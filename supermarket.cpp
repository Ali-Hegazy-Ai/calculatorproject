...
int main()
{
    ...
    cin.ignore(); // Using cin.ignore instead of getchar() for better compatibility
...
void buy_product()
{
    ...
    // Input validation - check for array bounds and valid input
    if (num_products <= 0 || num_products > 15) {
        cout << "Invalid number of products. Please enter a number between 1 and 15.\n";
        return;
    }
    ...
    do
    {
        ...
        {        case 1:
                for (int j = 0 ; j < num_products ; j++)
                {
                    ...
                    // Validate product number is within range
                    if (cin.fail() || products_bag[j] <= 0 || products_bag[j] > menu_size) {
                        cout << "Invalid product number. Please enter a number between 1 and " << menu_size << ".\n";
                        cin.clear(); // Clear error flags
                        cin.ignore(10000, '\n'); // Clear input buffer
                        j--; // Try again with this index
                        continue;
                    }
                }
...
void create()
{
    ...
    // Limit verification attempts to prevent stack overflow from recursive calls
    int attempts = 0;
    ...
    while (attempts < MAX_ATTEMPTS) {
        ...
        else
        {
            ...
            if (attempts < MAX_ATTEMPTS) {
                cout << "Wrong input. You have " << (MAX_ATTEMPTS - attempts) << " attempts remaining.\n";
            } else {
...
{
    int attempts = 0;
    const int MAX_ATTEMPTS = 3; // Limit login attempts
    ...
    while (attempts < MAX_ATTEMPTS) {
        ...
        else
        {
            attempts++;
            if (attempts < MAX_ATTEMPTS) {
                cout << "Invalid credentials. You have " << (MAX_ATTEMPTS - attempts) << " attempts remaining.\n";
            } else {
                cout << "Too many failed attempts. Returning to main menu.\n";
            }
        }
...
void Add()
{
    ...
    if (menu_size > 20) {
        cout << "Maximum menu size is 20. Setting size to 20.\n";
        menu_size = 20;
    } else if (menu_size <= 0) {
        cout << "Invalid menu size. Setting size to 1.\n";
        menu_size = 1;
    }
...
void Edit()
{
    ...
    for (int q = 0 ; q < num_products ; q++)
    {
        ...
        // Validate product number is within menu range
        if (product_num <= 0 || product_num > menu_size) {
            cout << "Invalid product number. Please enter a number between 1 and " << menu_size << ".\n";
            q--; // Try again for this iteration
            continue;
        }
